...
import mx.controls.Alert;
	...
	private function btnLancarRetirada_Click():void
	{
		...
		for each (var finanTipoLancamento:Finan_TipoLancamento in App.single.cache.arrayFinan_TipoLancamento)
		{
			if (finanTipoLancamento.nomeTipoLancamento != "TRANSFERENCIA DE CAIXA")
				continue;
			existe = true;
			break;
		}
		if (!existe)
		{
			Alert.show("Plano de Conta 'TRANSFERENCIA DE CAIXA' não cadastrado no sistema. Cadastre o mesmo para poder realizar a retirada", "Mensagem SDE", 4, null, null, Imagens.unica.icn_32_alerta);
			return;
		}
		...
		{
			Alert.show("A retirada não pôde ser realizada, saldo de caixa insuficiente.\nSaldo atual:"
		...
		*/
		...
		App.single.n.modificacoes.Caixa_RetiradaPagamento(cxL, finanLancamento,
			function ():void{
