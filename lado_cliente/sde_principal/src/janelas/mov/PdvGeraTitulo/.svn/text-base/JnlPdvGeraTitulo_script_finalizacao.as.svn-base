	...
	private function usuario_salva_venda_com_nf():void
	{
		...
		if (isNFe)
		{
			...
			{
				mov.vlrAcrescimo = indicador_venda_valor_final - nsDescFinal_valor.value;
		...
		if (mov.impressao == EMovImpressao.nfe_produto)
		{
			...
			var tipoNFE:String = Sessao.unica.parametrizacao.getParametro(Variaveis_SdeConfig.EMPRESA_PDV_NFEXML);
				...
				if(tipoNFE == "1")
				{
	                calculoValorFinal(nsDescFinal_valor.value);
	...
	private function sistema_salva_mov(msg:String):void /*ruthy*/
	{
		...
		for each (var cxL:Cx_Lancamento in arraycLancamentosCaixa)
		{
			...
			if (finanTipoPagamento.geraContasReceber && cliente_selecionado.id == 1 && cache.getFinan_Portador(finanTipoPagamento.idPortador).nome == "CARTEIRA")
			{
	...
	private function escreveArquivoPRZTEF():String
	{
		...
		var proximaIdMov:String = getProximaIdMov();
		
		var strRetorno:String = alinhaString("Nº" + proximaIdMov, 35) +
			...
			alinhaString(cli.cpf_cnpj + " " + ((cli.tipo == EPesTipo.Fisica) ? (end.inscr == "") ? "RG.:"+cli.rg : "I.E.:"+end.inscr : "I.E.:"+end.inscr), 35) +
	...
	private function getProximaIdMov():String
	{
		var retorno:String;
		...
		retorno = ((movimentacoes.getItemAt(0) as Mov).id + 1).toString();
		return retorno;
	}
	...
	private function verificaEstoque():Boolean
	{
		...
		{
			Alert.show("Venda não pode ser concluída, itens com estoque insuficiente:\n"+escreveItensSemEstoque(itensSemEstoque), "Mensagem SDE", 4, null, null, Imagens.unica.icn_32_alerta);
			return false;
		}
		else
			return true;
